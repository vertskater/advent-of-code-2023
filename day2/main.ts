...
function getSpecialChars(text: string): [string, number][] {
  ...
  specialChars.forEach((num) => {
    ...
    if (searchSubStringResult) returnData.push([num, onIndex]);
  });
  return returnData;
}
...
function getDigit(data: string, reverse: boolean) {
  ...
  if (!reverse) {
    for (let i = 0; i < data.length; i++) {
      ...
      if (!isNaN(firstNum)) return [data[i], i];
    }
  } else {
...
function calcWhoIsFirst(text: string, reverse: boolean) {
  ...
  if (!reverse) {
    ...
    if (indexTextChar.includes(numItem)) {
      const [num] =
        indicesSpecialChars.find((item) => item[1] === numItem) || [];
...
console.log(
  calcWhoIsFirst("zoneight234", false) +
    ", " +
